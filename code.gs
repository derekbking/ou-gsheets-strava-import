   ...
   var html = HtmlService.createHtmlOutput(
    ...
    ${
     ...
     <div style='display: flex; gap: 1rem; margin-bottom: .5rem;'>
       <button class="tab-button active" id='tab-laps-${activityIndex}' onclick="switchTab(${activityIndex}, 'laps')">Laps (${activityData.laps?.length ?? 0})</button>
       <button class="tab-button" id='tab-splits-${activityIndex}' onclick="switchTab(${activityIndex}, 'splits')">Splits (${activityData.splits?.length ?? 0})</button>
     </div>
